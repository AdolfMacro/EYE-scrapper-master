...
#
# ROLE:
#     Single GUI-facing boundary for EYES Master.
#
# CURRENT STATE:
#     این فایل تنها مرز ارتباط GUI با لایه Master است.
#     تمام عملیات مربوط به ساخت، اجرا، توقف، حذف، وضعیت،
#     snapshot و lifecycle اسکرپرها از طریق GUIController
#     به ProcessManager منتقل می‌شود.
#
# PROBLEMS:
#     1. مسیرهای متعدد و موازی برای normalize کردن create
#        وجود داشت.
#     2. _normalize_create_arguments() تعریف شده بود اما
#        در create_scraper() استفاده نمی‌شد.
#     3. kwargs در create_scraper() عملاً نادیده گرفته می‌شد.
#     4. منطق provider resolution در چند نقطه تکرار شده بود.
#     5. compatibility بین payload قدیمی و API جدید بیش از
#        حد در خود create_scraper() پخش شده بود.
#
# ARCHITECTURE:
#     GUI
#       ↓
...
#
#         One child scraper = exactly one provider.
#
#     Canonical keyword contract:
#
#         keywords -> list[str]
#         keyword  -> first keyword
#
# REQUIRED CHANGES:
#     1. یک مسیر واحد برای normalize کردن create.
#     2. حذف منطق تکراری provider/config.
#     3. ادغام صحیح payload و keyword arguments.
#     4. حفظ compatibility با API قدیمی.
#     5. ارسال فقط قرارداد نهایی و معتبر به ProcessManager.
#
# DECISION:
#     اصلاح و تثبیت به‌عنوان نسخه canonical.
#
# SCORE:
#     9.5/10
#
...
class GUIController:
    """
    Canonical GUI-facing controller for EYES Master.

    The GUI communicates with the Master layer through this
    class only.
    ...
    def _require_manager(self) -> ProcessManager:
        """
        Return the connected ProcessManager.
        """
    ...
    def _require_name(
        ...
        """
        Validate and normalize scraper name.
        """
    ...
    def _normalize_config(
        ...
        """
        Normalize scraper configuration.
        """
    ...
    def _validate_config(
        ...
        """
        Normalize canonical keyword fields.
    ...
    def _normalize_create_arguments(
        ...
        *,
        ...
        """
        Normalize every supported create_scraper() input form.
        ...
        if data is not None:
            ...
            if isinstance(
                ...
                raise TypeError(
                    "Create payload must be a dictionary."
                )
        ...
        payload_provider = payload.pop(
            "provider",
            None,
        )

        payload_providers = payload.pop(
            "providers",
            None,
        )

        payload_target = payload.pop(
            "target",
            None,
        )

        payload_database = payload.pop(
            "database",
            None,
        )

        payload_config = payload.pop(
            "config",
            None,
        )

        payload_log_file = payload.pop(
            "log_file",
            None,
        )

        payload_scraper_dir = payload.pop(
            "scraper_dir",
            None,
        )

        # ======================================================
        # EXPLICIT ARGUMENTS OVERRIDE PAYLOAD
        # ======================================================
        ...
        if database is None:
            database = payload_database

        if config is None:
            config = payload_config

        if log_file is None:
            log_file = payload_log_file

        if scraper_dir is None:
            scraper_dir = payload_scraper_dir

        # ======================================================
        # NAME
        # ======================================================

        normalized_name = self._require_name(
            name
        )
        ...
        # Remaining payload fields are legacy config fields.
        if payload:
        ...
        # Explicit extra config is highest priority.
        if extra_config:
    ...
    def create_scraper(
        self,
        name: Optional[str | dict[str, Any]] = None,
        providers: Any = None,
        target: Optional[str] = None,
        database: Optional[str] = None,
        config: Optional[dict[str, Any]] = None,
        ...
        **kwargs: Any,
        ...
        The scraper is validated completely before
        ProcessManager.create() is called.
        """

        # ------------------------------------------------------
        # kwargs are treated as legacy/extra config values.
        # ------------------------------------------------------
