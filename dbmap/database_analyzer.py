...
class DatabaseAnalyzer:
    ...
    The database is always opened using SQLite read-only mode.
    """
    ...
    def _readonly_uri(
        ...
        # SQLite URI paths need an absolute filesystem path.
        # urllib is intentionally avoided because sqlite3 URI
        # handling accepts the normal absolute filesystem form.
        return (
            "file:"
            + str(path)
            + "?mode=ro"
    ...
    def analyze_coordinates(
        ...
        quoted_latitude = (
            self._quote_identifier(latitude)
        )

        quoted_longitude = (
            self._quote_identifier(longitude)
        )
    ...
    def duplicate_rows(
        ...
        """
        Count duplicate complete rows.

        Uses GROUP BY instead of COUNT(DISTINCT col1, col2),
        because SQLite does not support the latter form.
        """
